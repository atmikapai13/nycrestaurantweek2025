...
export default function FilterBar({
  ...
  const hasYelpRatings = useMemo(() => {
    return allRestaurants.some(
      (r) => r.yelp_rating !== null && r.yelp_rating !== undefined
    );
  }, [allRestaurants]);
      ...
      >
        ...
        <div className="filter-row filter-row-markers">
          ...
          <FilterDropdown
            ...
            placeholder={
              !hasYelpRatings ? "⚠️ Rating data not available" : undefined
            }
          />
          ...
          {/* Participation Weeks Filter - Only show when Restaurant Week is active */}
          {restaurantWeekActive && (
            <FilterDropdown
              label="Participation Weeks"
              icon=""
              options={participationWeeksOptions}
              selectedValues={activeFilters["Participation Weeks"] || []}
              onChange={(values) =>
                onFilterChange("Participation Weeks", values)
              }
            />
          )}
