...
function AppContent() {
  const { isochroneRegionSlugs, setSelectedRestaurant } = useMap();
  ...
  const handleRestaurantWeekToggle = useCallback(() => {
    setRestaurantWeekActive((prev) => !prev);
  }, []);

  const handleHasMenuToggle = useCallback(() => {
    setHasMenuActive((prev) => !prev);
  }, []);

  const handleRemisRecsToggle = useCallback(() => {
    setRemisRecsActive((prev) => !prev);
  }, []);

  const handleHighReviewCountToggle = useCallback(() => {
